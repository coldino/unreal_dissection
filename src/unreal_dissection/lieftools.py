...
class Image:
    ...
    def __init__(self, src: lief.Binary | str | bytes):
        ...
        bin: lief.Binary = src if isinstance(src, lief.Binary) else cast(lief.Binary, lief.parse(src))
        self.binary = bin
        self.base = bin.imagebase

        # Create a map of section names to section objects
        self._sections = {cast(str, cast(lief.Section, section).name): cast(lief.Section, section) for section in bin.sections}
